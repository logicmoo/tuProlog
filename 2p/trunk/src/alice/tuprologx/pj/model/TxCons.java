 ...
 * @author Maurizio
 */
public class TxCons<H extends TxTerm<?>, R extends TxCompound<?>> extends TxCompound<TxCons<H, R>>
        implements Iterable<TxTerm<?>> {
    ...
    public static <Z extends TxCons<?, ?>> Z make(String f, TxTerm<?>[] termList) {
        if (termList.length == 1)
            ...
            return uncheckedCast(new TxCompound3<TxTerm<?>, TxTerm<?>, TxTerm<?>>(f, termList[0], termList[1],
                    termList[2]));
        else if (termList.length > 3)
    ...
    */
    ...
    private void initFromList(java.util.List<TxTerm<?>> termList) {
        if (!termList.isEmpty()) {
            ...
            _theRest = uncheckedCast(!termList.isEmpty()
                    ? uncheckedCast(new TxCons<TxTerm<?>, TxCompound<?>>(null, termList))
                    : uncheckedCast(new TxNil()));
            return;
        }
    ...
    static boolean matches(alice.tuprolog.Term t) {
        return (!(t.isVar()) && t.isCompound() && !t.isList());
    }
    ...
    public alice.tuprolog.TuStruct marshal() {
        ...
        return TuStruct.createTuStructA(_theName, termArray);
    }
    /*
