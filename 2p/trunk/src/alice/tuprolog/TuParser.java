 ...
 */
public class TuParser implements /*Castagna 06/2011*/IParser, /**/ Serializable {
     ...
     */
    public TuParser(OperatorManager op, InputStream theoryText) {
        this(theoryText);
        if (op != null)
            opManager = op;
    }
     ...
     * and what text to parse
     */
    public TuParser(OperatorManager op, String theoryText, HashMap<Term, Integer> mapping) {
     ...
     * and what text to parse
     */
    public TuParser(OperatorManager op, String theoryText) {
        this(theoryText);
        if (op != null)
            opManager = op;
    }
    ...
    public Term nextTerm(boolean endNeeded) throws InvalidTermException {
        try {
            ...
            throw new InvalidTermException("An I/O error occured.", tokenizer.offsetToRowColumn(getCurrentOffset())[0],
                    tokenizer.offsetToRowColumn(getCurrentOffset())[1] - 1);
            /**/
        }
    }
        ...
        for (; t.isOperator(commaIsEndMarker); t = tokenizer.readToken()) {
            ...
            if (YFX >= YF && YFX >= OperatorManager.OP_LOW) {
                ...
                if (ta != null) {
                    ...
                    leftSide = identifyTerm(YFX, TuFactory
                            .createTuStruct2(t.seq, leftSide.result, ta.result), tokenStart);
                    /**/
                    continue;
                }
            }
            ...
            if (YF >= OperatorManager.OP_LOW) {
                ...
                leftSide = identifyTerm(YF, S(t.seq, leftSide.result), tokenStart);
                /**/
                continue;
            }
            break;
        ...
        for (; operator.isOperator(commaIsEndMarker); operator = tokenizer.readToken()) {
            ...
            if (XFX >= XFY && XFX >= XF && XFX >= left.priority) { //XFX has priority
                ...
                if (found != null) {
                    ...
                    left = identifyTerm(XFX, TuFactory
                            .createTuStruct2(operator.seq, left.result, found.result), tokenStart);
                    /**/
                    continue;
                } else
            ...
            if (XFY >= XF && XFY >= left.priority) { //XFY has priority, or XFX has failed
                ...
                if (found != null) {
                    ...
                    left = identifyTerm(XFY, TuFactory
                            .createTuStruct2(operator.seq, left.result, found.result), tokenStart);
                    /**/
                    continue;
                }
            ...
            if (XF >= left.priority) //XF has priority, or XFX and/or XFY has failed
                ...
                return identifyTerm(XF, S(operator.seq, left.result), tokenStart);
            /**/
        ...
        if (f.isOperator(commaIsEndMarker)) {
            ...
            if (f.seq.equals("-")) {
                ...
                if (t.isNumber())
                    ...
                    return identifyTerm(0, TuParser.createNumber("-" + t.seq), tokenStart);
                /**/
                else
                    tokenizer.unreadToken(t);
            }
            ...
            if (FX >= FY && FX >= OperatorManager.OP_LOW) {
                ...
                if (found != null)
                    ...
                    return identifyTerm(FX, S(f.seq, found.result), tokenStart);
                /**/
                else
            ...
            if (FY >= OperatorManager.OP_LOW) {
                ...
                if (found != null)
                    ...
                    return identifyTerm(FY, S(f.seq, found.result), tokenStart);
                /**/
            }
    ...
    private Term expr0_list() throws InvalidTermException, IOException {
        ...
        TuToken t = tokenizer.readToken();
        if (",".equals(t.seq))
            return createTuCons(head, expr0_list());
        if ("|".equals(t.seq))
            return createTuCons(head, expr(true));
        if ("]".equals(t.seq)) {
            tokenizer.unreadToken(t);
            return createTuCons(head, createTuEmpty());
        }
        /*Castagna 06/2011*/
