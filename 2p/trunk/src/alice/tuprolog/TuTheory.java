...
public class TuTheory implements Serializable {
     ...
     */
    public TuTheory(InputStream is) throws IOException {
     ...
     */
    public TuTheory(String theory) throws InvalidTheoryException {
        if (theory == null) {
    ...
    public Iterator<? extends Term> iterator(TuProlog engine) {
        if (isTextual())
     ...
     */
    public void append(TuTheory th) throws InvalidTheoryException {
        if (th.isTextual() && isTextual()) {
            ...
            if (clauseList.isEmptyList())
                ...
                while (!(q = (TuStruct) p.getArg(1)).isEmptyList())
                    p = q;
    ...
    //Alberto
  	...
  	//Alberto
  	public static TuTheory fromJSON(String jsonString){
  		return JSONSerializerManager.fromJSON(jsonString, TuTheory.class);	
  	}
