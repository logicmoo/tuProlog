 ...
 */
public class TuTokenizer extends StreamTokenizer implements Serializable {
    ...
    static {
        Arrays.sort(TuTokenizer.GRAPHIC_CHARS);  // must be done to ensure correct behavior of Arrays.binarySearch
    }
    ...
    private LinkedList<TuToken> tokenList = new LinkedList<TuToken>();
    ...
    public TuTokenizer(String text) {
     ...
     */
    public TuTokenizer(Reader text) {
        super(text);
     ...
     */
    /*Castagna 06/2011*/public/**/ TuToken readToken() throws InvalidTermException, IOException {
     ...
     */
    void unreadToken(TuToken token) {
        tokenList.addFirst(token);
    }

    TuToken readNextToken() throws IOException, InvalidTermException {
        int typea;
        String svala;
        if (pushBack2 != null) {
        ...
        while (TuTokenizer.isWhite(typea)) {
        	/*Castagna 06/2011*/
            //typea = super.nextToken();
            typea = tokenConsume();
            /**/
        ...
        if (typea == TT_EOF) return new TuToken("", TuTokenizer.EOF);
        if (typea == '(') return new TuToken("(", TuTokenizer.LPAR);
        if (typea == ')') return new TuToken(")", TuTokenizer.RPAR);
        if (typea == '{') return new TuToken("{", TuTokenizer.LBRA2);
        if (typea == '}') return new TuToken("}", TuTokenizer.RBRA2);
        if (typea == '[') return new TuToken("[", TuTokenizer.LBRA);
        if (typea == ']') return new TuToken("]", TuTokenizer.RBRA);
        if (typea == '|') return new TuToken("|", TuTokenizer.BAR);

        if (typea == '!') return new TuToken("!", TuTokenizer.ATOM);
        if (typea == ',') return new TuToken(",", TuTokenizer.OPERATOR);
        ...
        if (typea == '.')  { // check that '.' as end token is followed by a layout character, see ISO Standard 6.4.8 endnote
        	...
        	int typeb = tokenConsume();
        	/**/
            if (TuTokenizer.isWhite(typeb) || typeb == '%' || typeb == StreamTokenizer.TT_EOF)
                return new TuToken(".", TuTokenizer.END);
            else
            	/*Castagna 06/2011*/
                //pushBack();
            	tokenPushBack();
        ...
        if (typea == TT_WORD) {
            ...
            if (Character.isUpperCase(firstChar) || '_' == firstChar)
                return new TuToken(svala, TuTokenizer.VARIABLE);
            	...
            	/**/
                if (typeb == '(')
                    return new TuToken(svala, TuTokenizer.ATOM | TuTokenizer.FUNCTOR);
                if (TuTokenizer.isWhite(typeb))
                    return new TuToken(svala, TuTokenizer.ATOM | TuTokenizer.OPERATOR);
                return new TuToken(svala, TuTokenizer.ATOM);
            }
        }
        ...
        if (typea == '\'' || typea == '\"' || typea == '`' ) {
            ...
            if (qType == SQ_SEQUENCE) {
                ...
                /**/
                if (typeb == '(')
                    return new TuToken(quoteBody, qType | FUNCTOR);
            }
            return new TuToken(quoteBody, qType);
        }

        // symbols
        if (Arrays.binarySearch(TuTokenizer.GRAPHIC_CHARS, (char) typea) >= 0) {
            ...
            int typeb = typea;
            // String svalb = null;
            while (Arrays.binarySearch(TuTokenizer.GRAPHIC_CHARS, (char) typeb) >= 0) {
...
//            }
            return new TuToken(symbols.toString(), TuTokenizer.OPERATOR);
        }
            ...
            try { // the various parseInt checks will throw exceptions when parts of numbers are written illegally
                ...
                if (svala.startsWith("0")) {
                    if (svala.indexOf('b') == 1)
                        return new TuToken("" + java.lang.Long.parseLong(svala.substring(2), 2), TuTokenizer.INTEGER); // try binary
                    if (svala.indexOf('o') == 1)
                        return new TuToken("" + java.lang.Long.parseLong(svala.substring(2), 8), TuTokenizer.INTEGER); // try octal
                    if (svala.indexOf('x') == 1)
                        return new TuToken("" + java.lang.Long.parseLong(svala.substring(2), 16), TuTokenizer.INTEGER); // try hex
                }
                ...
                if (typeb != '.' && typeb != '\'') { // i.e. not float or character constant
                	...
                	/**/
                    return new TuToken("" + java.lang.Long.parseLong(svala), TuTokenizer.INTEGER);
                }
                ...
                if (typeb == '\'' && "0".equals(svala)) {
                    ...
                    if ((intVal = isCharacterCodeConstantToken(typec, svalc)) != -1)
                        return new TuToken("" + intVal, TuTokenizer.INTEGER);
                ...
                if (typec != TT_WORD) { // if its not, the period is an End period
                    ...
                    return new TuToken(svala, INTEGER); // return what must be an int
                }
                    ...
                    if (exponent == svalc.length() - 1) {             // the exponent must be signed exponent
                        ...
                        if (typeb2 == '+' || typeb2 == '-') {
                            ...
                            if (typec2 == TT_WORD){
                                ...
                                return new TuToken(svala + "." + svalc + (char) typeb2 + svalc2, TuTokenizer.FLOAT);
                            }
                        }
                    }
                }
                ...
                return new TuToken(svala + "." + svalc, TuTokenizer.FLOAT);
