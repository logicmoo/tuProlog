...
loadgameCS :- load_convention('Conventions.dll','Conventions.CSharpConvention',CSConv), 
	new_object(CSConv, 'TTTCS.TicTacToe, TTTCS', [], Game),
	go(Game).
	
loadgameFS :- load_convention('Conventions.dll','Conventions.FSharpConvention',FSConv), 
	new_object(FSConv, 'TTTFS.TicTacToe, TTTFS', [], Game),
	go(Game).
	
loadgameVB :- load_convention('Conventions.dll','Conventions.VBDotNetConvention',VBConv),  
...
get_board(Game, Board) :- Game.board <- get(BoardText), text_term(BoardText, Board).	
get_board(Game, Board) :- Game <- getBoard returns BoardText, text_term(BoardText, Board).

% predicati per recuperare il numero di caselle vuote
get_remaining(Game, Open) :- Game.remaining <- get(Open).
get_remaining(Game, Open) :- Game <- getRemaining returns Open.

go(Game) :- print('Playing tic-tac-toe.'), nl, 
	explainMoves(Game), game(Game), nl, nl.

explainMoves(Game) :- print('You choose a move by giving the number of the'), nl, 
	print('square to which you wish to move:'), nl, nl,
	Game <- printBoard. 

game(Game) :- print('Do you wish to be x or o? '), Game <- inputPlayer returns Player,
	print('Do you want to go first (y/n)? '), Game <- inputResponse returns PlayerMoves,
	play(Player, PlayerMoves, Game).

play(Player, PlayerMoves, Game) :- oneMove(Player, PlayerMoves, Game), 
	negate(PlayerMoves, NewPlayerMoves), 
	get_remaining(Game, Open),
	continuePlay(Player, NewPlayerMoves, Open, Game).

% Il giocatore umano effettua la mossa
oneMove(Player, yes, Game) :- 
	Game <- inputMove returns Location, 
	makeMove(Location, Player, Game).  
	
% Il computer effettua la mossa
oneMove(Player, no, Game) :- 
	opposite(Player, Computer),
	get_board(Game, Board),
	generateMove(Computer, Board, Location), 
	print('My move: '), print(Location), nl,
	makeMove(Location, Computer, Game).  

% viene effettuata la mossa invocando il metodo gioca sull'oggetto
makeMove(Location, Val, Game) :- Game <- gioca(Location, Val), Game <- printBoard.
	
% Predicati che controllano l'esito della partita
continuePlay(Player,_,_, Game) :- won(Who, Game), !, winnerOut(Who, Player).

continuePlay(Player, PlayerMoves, NumOpen, Game) :- NumOpen == 0, print('The game is a draw'), nl.

continuePlay(Player, PlayerMoves, NumOpen, Game) :- play(Player, PlayerMoves, Game).
